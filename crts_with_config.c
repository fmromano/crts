...
struct CognitiveEngine {
    char modScheme[20];
    double default_tx_power;
    char option_to_adapt[20];
    char goal[20];
    double threshold;
    double latestGoalValue;
    int iterations;
    int payloadLen;
...
int config_cog_engine(struct CognitiveEngine * ce)
{
    ...
    {
        /*Read the string*/
        if (config_setting_lookup_string(setting, "option_to_adapt", &str2))
        {
            strcpy(ce->option_to_adapt,str2);
            printf ("%s",str2);
        }
        else
            printf("\nNo 'param2' setting in configuration file.");
       
        if (config_setting_lookup_string(setting, "goal", &str3))
        {
            strcpy(ce->goal,str3);
            printf ("%s",str3);
        }
        if (config_setting_lookup_string(setting, "modScheme", &str4))
        {
            strcpy(ce->modScheme,str4);
            printf ("Modulation Scheme:%s",str4);
        }
        /*Read the integer*/
        if (config_setting_lookup_int(setting, "iterations", &tmp))
        {
           ce->iterations=tmp;
           printf("\nIterations: %d", tmp);
        }
        if (config_setting_lookup_int(setting, "payloadLen", &tmp2))
        {
           ce->payloadLen=tmp2; 
           printf("\nPayloadLen: %d", tmp2);
        }
        if (config_setting_lookup_int(setting, "numSubcarriers", &tmp3))
        {
           ce->numSubcarriers=tmp3; 
           printf("\nNumber of Subcarriers: %d", tmp3);
        }
        if (config_setting_lookup_int(setting, "CPLen", &tmp4))
        {
           ce->CPLen=tmp4; 
           printf("\nCPLen: %d", tmp4);
        }
        if (config_setting_lookup_int(setting, "taperLen", &tmp5))
        {
           ce->payloadLen=tmp5; 
           printf("\nPayloadLen: %d", tmp5);
        }
        /*Read the floats*/
        if (config_setting_lookup_float(setting, "default_tx_power", &tmp6))
        {
           ce->default_tx_power=tmp6; 
           printf("\nDefault Tx Power: %f", tmp6);
        }
        if (config_setting_lookup_float(setting, "latestGoalValue", &tmp7))
        {
           ce->latestGoalValue=tmp7; 
           printf("\nLatest Goal Value: %f", tmp7);
        }
        if (config_setting_lookup_float(setting, "threshold", &tmp8))
        {
           ce->threshold=tmp8; 
           printf("\nThreshold: %f", tmp8);
        }
...
int config_scenario(struct Scenario * sc)
{
    ...
    if (setting != NULL)
    {
        ...
        /*Read the integer*/
        if (config_setting_lookup_int(setting, "addNoise", &tmp))
        {
            printf("\nAddnoise: %d", tmp);
            sc->addNoise=tmp;
        }
        ...
        /*Read the integer*/
        if (config_setting_lookup_int(setting, "noiseSNR", &tmp2))
        {
            printf("\nNoise SNR: %d", tmp2);
            sc->noiseSNR=tmp2;
        }
        ...
        /*Read the integer*/
        if (config_setting_lookup_int(setting, "noiseDPhi", &tmp3))
        {
            sc->noiseDPhi=tmp3;
            printf("\nNoiseDPhi: %d", tmp3);
        }
        ...
        /*Read the integer*/
        if (config_setting_lookup_int(setting, "addInterference", &tmp4))
        {
            sc->addInterference=tmp4;
            printf("\naddInterference: %d", tmp4);
        }
        else
            printf("\nNo addInterference setting in configuration file.");
        /*Read the integer*/
        if (config_setting_lookup_int(setting, "addFading", &tmp5))
        {
            sc->addFading=tmp5;
            printf("\naddFading: %d", tmp5);
        }
